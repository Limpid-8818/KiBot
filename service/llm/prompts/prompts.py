...
FUNCTION_CALLING_INTENT_PROMPT = """
    - 任务：
        你是一个智能助手，需要判断用户的查询是否需要调用工具，以及调用哪个工具。
        如果需要调用工具，请选择最合适的工具并确定所需参数。
        请根据提供的工具列表进行判断。
    - 可用工具详细信息列表：
        {tools}
    - 返回格式：
        ...
        {{
          ...
          "tool_name": "工具名称",        // 如果需要调用工具，填写工具名称，否则为null
          "tool_parameters": {{ ... }},     // 如果需要调用工具，填写参数，否则为null
        ...
        2. 只能使用提供的工具列表中的工具
        3. 检查工具参数是否完整，确保符合工具的参数要求
        4. confidence字段表示你对这个判断的信心程度，1.0表示最确定
